...
let updateContactInfo = (data, type, mode) => {
    switch(type) {
        ...
        // change twitter
        case "twitter":
            twitter.innerHTML = data.twitter_user_name 
        ...
        fetch("https://api.github.com/users/" + user).then((res) => {
            ...
            res.json().then((data) => {
                ...
                document.getElementsByClassName("user-profile-img")[0].src =data.avatar_url;
                				...
                				// Update the name
                document.getElementsByClassName("full-name")[0].innerHTML = data.name ? data.name
                : data.login;
				// Update the username
                document.getElementsByClassName("username")[0].innerHTML = "@"+ data.login;

                // Update the date joined
                document.getElementsByClassName("user-bio")[0].innerHTML = data.bio ? data.bio 
            ...
            // If search is unsuccessful, return the error message
                document.getElementsByClassName("err-mess")[0].style.display = "block";
            }
        });
});
